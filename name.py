...
def load():
    glo.lock("db")
    global current_images_encoded
    current_images_encoded = []
    global images
    images = []
    data, x, y = db.get_all_info()
    for i in range(x):
        images.append(data[i][2])
        fout = open("temporary.png", "wb")
        fout.write(data[i][1])
        current_image = fr.load_image_file("temporary.png")
        current_images_encoded.append(fr.face_encodings(current_image)[0])
    glo.release("db")
    ...
    def run(self):
        while True:
            ...
            try:
                glo.lock("face_now")
                r_exist_one_face = glo.get_value("exist_one_face")
                # (<class 'PIL.UnidentifiedImageError'>, UnidentifiedImageError("cannot identify image file
                # 'face_now.png'"), <traceback object at 0x000001D24E062FC8>)
                ...
                if exist_one_face:
                    ...
                    if eye_open and eye_close:
                        ...
                        for i in range(len(images)):
                            #print(len(images))
                            result = fr.compare_faces([face_img_encoded], current_images_encoded[i],
                                                      tolerance=0.39)
                            if result[0]:
                                self.set_name("匹配：" + images[i])
                                get_name = True
                                break
                        glo.release("db")
                        if not get_name:
